...
# data_paths = [os.path.join(wd, 'ncaa_mod', path) for path in data_paths]
...
#         self.num_levels = 1
#         dilation_sizes = [2 ** i for i in range(self.num_levels)]
        
#         in_channels = input_size  # Change this line

#         out_channels = num_channels
#         print(type(in_channels), type(num_channels), type(kernel_size), type(dilation_sizes), type(dropout), type(out_channels))
#         self.temporal_block = TemporalBlock(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_sizes,
#                                             padding=[(kernel_size-1) * d for d in dilation_sizes], dropout=dropout)
